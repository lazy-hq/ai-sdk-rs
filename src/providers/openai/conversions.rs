...
use crate::core::language_model::{LanguageModelResponseContentType, ReasoningEffort, Usage};
...
impl From<LanguageModelRequest<OpenAI>> for CreateResponse {
    fn from(request: LanguageModelRequest<OpenAI>) -> Self {
        let mut items: Vec<InputItem> = request
            .messages
            .into_iter()
        ...
        let tools: Option<Vec<ToolDefinition>> = request.tools.map(|t| {
            t.tools
                .lock()
...
// mod tests {
//     use super::*;
//     use crate::core::language_model::{ReasoningEffort, Usage};
